...
class Launchpad_X(InstrumentControlMixin, NovationBase):
    ...
    def on_identified(self, midi_bytes):
        ...
        for control_surface in Live.Application.get_application().control_surfaces:
            ...
            logger.warning('LP_Control_Surface %s'  % name )
            if name.find('Launchpad_X_for_SONG_MODE') > -1:
                logger.warning('FOUND DAW')
                self.LP_X_DAW = control_surface
            if name.find('SONG_MODE_Launchpad_X') > -1:
                logger.warning('FOUND MIDI')
                self.LP_X_MIDI = control_surface
    ...
    def _create_session_modes(self):
        ...
        (
         self._session_modes.add_mode('overview', (
          ...
          AddLayerMode(self._session_navigation, Layer(page_up_button='up_button', page_down_button='down_button', page_left_button='left_button', page_right_button='right_button')),
    ...
    def _create_note_modes(self):
        ...
        self._note_modes.add_mode('scale', (
         ...
         AddLayerMode(self._background, layer=Layer(up_button='up_button', down_button='down_button', left_button='left_button', right_button='right_button'))))
    ...
    def __on_layout_switch_value(self, value):
        if int(value[0]) > 3:
            self.LP_X_DAW._send_midi((240, 0, 32, 41, 2, 12, 14, 1, 247))
            time.sleep(.01)
            self.LP_X_MIDI.LP_last_layout_byte(value)
            self.LP_X_MIDI.is_active_changed(1)
        else:
